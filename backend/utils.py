...
def generate_docx(data):
    try:
        ...
        # Add the second word
        run2 = left_paragraph.add_run(f"GZ IPZ: {data.passport_id if data.passport_id else ''}\n \n")
        ...
        # Add the second word
        run2 = left_paragraph.add_run(f"    {data.name if data.name else ''}, {data.passport_birthdate if data.passport_birthdate else ''}")
        run2.alignment = 3  # Set alignment if desired
        ...
        p = document.add_paragraph(f"{data.name if data.name else ''}")
        p.alignment = 2
        ...
        response['Content-Disposition'] = f'attachment; filename={str(data.id)}.docx'
